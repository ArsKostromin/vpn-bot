...
from bot.keyboards.main_menu import inline_main_menu  # импорт клавиатуры
...
async def process_start(user_id, username, respond_to):
    ...
    if user:
        await respond_to.bot.send_photo(
            ...
            photo="https://play-lh.googleusercontent.com/BFkf2bgtxsCvsTnR2yw8yuWD3mgpThoyiRoBhoazTqFFMNOmdxGAAqS7vMATyNwelQ",  # замени на свою ссылку или путь к файлу
            caption=(
...
#         user_uuid = str(uuid.uuid4())
#         vless_link = generate_vless_link(user_uuid)
#         await create_user(message.from_user.id, message.from_user.username, vless_link)

#         await message.answer(
#             text=(
#                 f"✅ Добро пожаловать в YouFast VPN™, {message.from_user.full_name}!\n\n"
#                 "🔧 Ваш VPN УЖЕ готов к работе и будет доступен **БЕСПЛАТНО три дня!**\n\n"
#                 "📲 Установите приложение для вашей OS:\n\n"
#                 "🍏 iOS: [neroVPN](https://)\n"
#                 "🤖 Android: [neroVPN](https://)\n"
