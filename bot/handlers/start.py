...
async def process_start(
    ...
    # Проверяем, есть ли у пользователя reply-клавиатура (если чат пустой или нет клавиатуры)
    has_reply_keyboard = False
    try:
        # Если пользователь нажал кнопку "Главное меню", значит клавиатура есть
        if respond_to.text == "Главное меню":
            has_reply_keyboard = True
    except:
        pass

    # Если клавиатуры нет — отправляем "Авторизация..." с клавиатурой
    ...
    try:
        await respond_to.bot.send_photo(
            ...
            caption=(
                "🔥 Наш VPN обеспечивает высокую скорость без ограничений — Звонки Telegram и WhatsApp  без задержек и зависаний. YouTube   в высоком качестве\n\n"
                "🛡 Двойное шифрование данных (Double VPN) — ваш трафик под надежным шифрованием.\n\n"
                "🕵️ Полная анонимность — мы не делимся данными с МВД\n\n"
                "💥 Лучшая цена на рынке, не перегружаем сервера ради выгоды!\n\n"
                "✅ Наша группа : https://t.me/anonix_vpn"
            ),
        ...
        if error:
            ...
            await callback.message.answer(
                text=(
                    ...
                    "🚀 Теперь вы можете пользоваться VPN целых 10 дней бесплатно!\n\n"
