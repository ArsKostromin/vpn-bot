...
async def show_confirmation(callback: CallbackQuery, state: FSMContext):
    ...
    text += (
        ...
        f"Оформляя подписку, вы подтверждаете согласие на регулярные списания (автопродление), обработку персональных данных и принимаете условия публичной оферты."
        f"\n\n🔒 Списание средств будет происходить автоматически по окончании оплаченного периода до момента отмены подписки."
    )
